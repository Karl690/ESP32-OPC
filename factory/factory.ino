...
bool wifi_connect()
{
    ...
    Serial.printf("Connecting to %s...", wifi_config.sta.ssid);
...
void setup()
{
   nvs_flash_init();
    ...
    //  uint16_t* color_data = (uint16_t*)heap_caps_malloc(LVGL_LCD_BUF_SIZE * sizeof(uint16_t), MALLOC_CAP_DMA | MALLOC_CAP_INTERNAL);
    //  for(int i = 0; i < EXAMPLE_LCD_V_RES; i ++){
    //     for(int j = 0; j < EXAMPLE_LCD_H_RES; j ++) {
    //         color_data[i * EXAMPLE_LCD_H_RES + j] = 0xF033;
    //     }
    // }
    // esp_lcd_panel_draw_bitmap(panel_handle, 10, 10, 100, 100, color_data);
